...
- **4 ✅❌ code examples covering the 4 most error-prone patterns**: Slide transition (AnimatePresence vs CSS), Ken Burns (keyed motion.div vs CSS animation), auto-play cleanup (useEffect return vs leaked interval), and swipe (Framer drag vs native touch). Each explains the _why_ behind the failure.
